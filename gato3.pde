...
void setup(){
  ...
  for(int i = 0; i < pixels.length; i++){
    ...
    float ri = map(rx.valor(map(rojo,0,255,0,TWO_PI)),rx.min,rx.max,0,255);
    float vi = map(gx.valor(map(verde,0,255,0,TWO_PI)),gx.min,gx.max,0,255);
    float az = map(bx.valor(map(azul,0,255,0,TWO_PI)),bx.min,bx.max,0,255);;


    pixels[i] = color(ri,vi,az);
  }
  updatePixels();
}
