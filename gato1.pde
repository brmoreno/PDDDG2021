...
void setup(){
  ...
  ry = new OsciladorIr(15);
  gy = new OsciladorIr(15);
  by = new OsciladorIr(15);
  loadPixels();
  for(int i = 0; i < pixels.length; i++){
    ...
    float azul = blue(ci);
    rojo += map(rx.valor(map(i%gato.width,0,width,0,TWO_PI)),rx.min,rx.max,-100,100);
    verde += map(gx.valor(map(i%gato.width,0,width,0,TWO_PI)),gx.min,gx.max,-50,50);
    azul += map(bx.valor(map(i%gato.width,0,width,0,TWO_PI)),bx.min,bx.max,-100,100);
    rojo += map(ry.valor(map(i/gato.width,0,width,0,TWO_PI)),ry.min,ry.max,-100,100);
    verde += map(gy.valor(map(i/gato.width,0,width,0,TWO_PI)),gy.min,gy.max,-50,50);
    azul += map(by.valor(map(i/gato.width,0,width,0,TWO_PI)),by.min,by.max,-100,100);
